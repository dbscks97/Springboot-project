...
<form method="get" action="advice_bbs_list">
	<script
...
<div class="row">
	...
	<div class="col-md-7 qt">	
        ...
        <tr>
          <td>${b.bbs_no}</td>
          <td>
          <a href="advice_bbs_cont?bbs_no=${b.bbs_no}&state=cont&page=${page}" style="text-decoration:none;color:black;" >${b.bbs_title}</a></td>
          <td>${b.bbs_name}</td>
          <td>${b.bbs_date}</td>
          <td>${b.bbs_hit}</td>   
        </tr>
      ...
      <c:if test="${empty blist}">
		...
		onclick="location='advice_bbs_write?page=${page}';">글작성</button>
	</div>
	</div>
  ...
  <div class="pageNum" style="margin-top: 50px">
    ...
    <c:if test="${(empty find_field) && (empty find_name)}">
     ...
     <c:if test="${page>1}">
      <a href="advice_bbs_list?page=${page-1}">[이전]</a>&nbsp;
     </c:if>
     ...
     <c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
       ...
       <c:if test="${a !=page}"> <%--현재 쪽번호가 선택 안된 경우 --%>
        <a href="advice_bbs_list?page=${a}">[${a}]</a>&nbsp;
       </c:if>
     </c:forEach>
     
     <c:if test="${page >= maxpage}">[다음]</c:if>
     <c:if test="${page < maxpage}">
      <a href="advice_bbs_list?page=${page+1}">[다음]</a>
     </c:if>
    </c:if>
    ...
    <%-- 검색후 페이징 --%>
     ...
     <c:if test="${page>1}">
      <a href="advice_bbs_list?page=${page-1}&find_field=${find_field}&find_name=${find_name}">[이전]</a>&nbsp;
     </c:if>
     ...
     <c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
       ...
       <c:if test="${a !=page}"> <%--현재 쪽번호가 선택 안된 경우 --%>
        <a href="advice_bbs_list?page=${a}&find_field=${find_field}&find_name=${find_name}">[${a}]</a>&nbsp;
       </c:if>
     </c:forEach>
     
     <c:if test="${page >= maxpage}">[다음]</c:if>
     <c:if test="${page < maxpage}">
      <a href="advice_bbs_list?page=${page+1}&find_field=${find_field}&find_name=${find_name}">[다음]</a>
     </c:if>
    </c:if>
       </ul>
    </nav>     
